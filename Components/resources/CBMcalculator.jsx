...
/* Air freight divisors */
const DIVISORS = [
  { value: 6000, label: "6000 (Air Cargo — IATA)",    sub: "1 CBM = 166.67 kg",  who: "Standard air freight" },
  { value: 5000, label: "5000 (Courier / Express)",   sub: "1 CBM = 200 kg",     who: "DHL / FedEx / UPS"    },
];

/* Pallet presets (L × W in cm, unit always cm) */
const PALLET_PRESETS = [
  { label: "Euro Pallet",  l: "120", w: "80",  unit: "cm" },
  { label: "US Pallet",    l: "121.9", w: "101.6", unit: "cm" },
  { label: "UK Pallet",    l: "120",  w: "100", unit: "cm" },
];

/* Density classification thresholds (kg / CBM) */
function getDensityClass(kgPerCBM) {
  if (!isFinite(kgPerCBM) || kgPerCBM <= 0) return null;
  if (kgPerCBM < 100)  return { label: "Lightweight",    color: T.orange,  icon: "⬆", tip: "Volumetric weight will almost certainly govern — consider re-packing to reduce volume."     };
  if (kgPerCBM < 300)  return { label: "Standard",       color: T.blue,    icon: "⬌", tip: "Moderate density — verify against divisor to confirm which weight governs."                 };
  if (kgPerCBM < 600)  return { label: "Dense",          color: T.green,   icon: "⬇", tip: "Actual gross weight likely governs air freight. Ideal density for cost efficiency."          };
  return                      { label: "Very Dense",      color: T.purple,  icon: "⚡", tip: "Extremely dense — double-check container floor load limits (typically 3–5 t/m²)."          };
}
...
/* ── Validation helpers ── */
function isPositive(v) { return parseFloat(v) > 0; }
function dimError(v)   { return v !== "" && parseFloat(v) <= 0; }

function calcItem(item) {
  const f = UNIT_TO_M[item.unit] || 0.01;
  const l = (parseFloat(item.l) || 0) * f;
  const w = (parseFloat(item.w) || 0) * f;
  const h = (parseFloat(item.h) || 0) * f;
  const cbmEach  = l * w * h;
  const qty      = Math.max(parseInt(item.qty)   || 0, 0);
  const wt       = Math.max(parseFloat(item.wt)  || 0, 0);
...
function NumIn({ value, onChange, placeholder, hasError = false, style = {} }) {
  const borderColor = hasError ? T.red : T.border;
  return (
    <input
      type="number"
      min={0}
      ...
      style={{ width: "100%", background: hasError ? T.redLt : T.card, border: `1.5px solid ${borderColor}`, borderRadius: 9, padding: "9px 10px", fontSize: 13, color: T.text, fontFamily: T.fontMono, fontWeight: 600, outline: "none", transition: "border-color .18s", boxSizing: "border-box", ...style }}
    />
  );
}
...
function ContainerBar({ container, totalCBM, totalKg }) {
  const cp = Math.min((totalCBM / container.cbm) * 100, 100);
  const kp = Math.min((totalKg / container.maxKg) * 100, 100);
  ...
  return (
    <div style={{ background: T.cardAlt, border: `1.5px solid ${fits ? T.border : "#fca5a5"}`, borderRadius: 12, padding: "13px 15px", marginBottom: 10 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: fits ? T.green : T.red, fontFamily: T.fontSans, background: fits ? T.greenLt : T.redLt, padding: "3px 10px", borderRadius: 99 }}>
          {fits ? "✓ Fits" : "✗ Oversize"}
        </span>
      </div>
      {[
        { label: "Volume", pct: cp, fmt: `${fmtC3(totalCBM)} / ${container.cbm} CBM` },
        { label: "Weight", pct: kp, fmt: `${fmtN(totalKg)} / ${fmtN(container.maxKg)} kg` },
      ].map(row => (
...
function ChargeBar({ actual, volumetric, unit, label }) {
  const max = Math.max(actual, volumetric, 0.001);
  const ap = (actual / max) * 100, vp = (volumetric / max) * 100;
  const vh = volumetric > actual;
  return (
    <div style={{ marginBottom: 12 }}>
      {[
        { label: "Gross wt.", pct: ap, hi: !vh, val: actual },
        { label: "Vol. wt.",  pct: vp, hi: vh,  val: volumetric },
      ].map(r => (
        <div key={r.label} style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 5 }}>
          ...
          <span style={{ fontSize: 11, fontFamily: T.fontMono, color: T.textMid, width: 90 }}>{fmtKg(r.val)} {unit}</span>
        </div>
      ))}
      <p style={{ fontSize: 11, color: vh ? T.orange : T.navy, fontFamily: T.fontSans, fontWeight: 600, marginTop: 4 }}>
        {vh
          ? "⚑ Volumetric weight is higher — freight charged on volume."
...
/* ─── Density Card ────────────────────────────────────────────────── */
function DensityCard({ totalKg, totalCBM, divisor }) {
  const density      = totalCBM > 0 ? totalKg / totalCBM : 0;
  const kgPerCBM     = 1000000 / divisor;          // e.g. 166.67 or 200
  const governs      = density >= kgPerCBM ? "actual weight" : "volumetric weight";
  const cls          = getDensityClass(density);

  if (!cls || totalCBM === 0) return null;

  return (
    <div style={{ background: T.card, border: `1.5px solid ${T.border}`, borderRadius: T.radius, padding: 20, boxShadow: T.shadow, marginBottom: 16 }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: T.textDim, letterSpacing: "0.09em", textTransform: "uppercase", fontFamily: T.fontSans, marginBottom: 14 }}>Cargo Density Analysis</p>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div>
          <p style={{ fontSize: 28, fontWeight: 800, color: cls.color, fontFamily: T.fontMono, letterSpacing: "-0.03em" }}>
            {fmtDec(density, 1)} <span style={{ fontSize: 14, color: T.textMid }}>kg/m³</span>
          </p>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 5, background: `${cls.color}15`, border: `1px solid ${cls.color}40`, borderRadius: 99, padding: "3px 10px", marginTop: 5 }}>
            <span style={{ fontSize: 11, fontWeight: 700, color: cls.color, fontFamily: T.fontSans }}>{cls.icon} {cls.label}</span>
          </div>
        </div>

        {/* Breakeven indicator */}
        <div style={{ textAlign: "right" }}>
          <p style={{ fontSize: 10, color: T.textDim, fontFamily: T.fontSans, marginBottom: 3, textTransform: "uppercase", fontWeight: 700, letterSpacing: "0.07em" }}>Air breakeven</p>
          <p style={{ fontSize: 16, fontWeight: 700, color: T.navy, fontFamily: T.fontMono }}>{fmtDec(kgPerCBM, 2)} kg/m³</p>
          <p style={{ fontSize: 10, color: T.textDim, fontFamily: T.fontSans, marginTop: 2 }}>÷{divisor} divisor</p>
        </div>
      </div>

      {/* Visual scale */}
      <div style={{ position: "relative", height: 10, background: `linear-gradient(to right, ${T.orange}, ${T.blue}, ${T.green}, ${T.purple})`, borderRadius: 99, marginBottom: 6 }}>
        <div style={{
          position: "absolute",
          top: "50%",
          left: `${Math.min((density / 700) * 100, 97)}%`,
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
        {["0", "100", "300", "600", "700+"].map(l => (
          <span key={l} style={{ fontSize: 9, color: T.textFnt, fontFamily: T.fontMono }}>{l}</span>
        ))}
      </div>

      <div style={{ display: "flex", gap: 8, background: `${cls.color}0d`, border: `1px solid ${cls.color}30`, borderRadius: 10, padding: "9px 12px" }}>
        <Info style={{ width: 13, height: 13, color: cls.color, flexShrink: 0, marginTop: 1 }} />
        <p style={{ fontSize: 11.5, color: T.textMid, lineHeight: 1.65, fontFamily: T.fontSans }}>
          {cls.tip} For air freight with ÷{divisor}: <strong style={{ color: cls.color }}>{governs} governs</strong>.
        </p>
      </div>
    </div>
  );
...
function ItemCard({ item, idx, onChange, onDelete, canDelete, calc }) {
  const hasLErr = dimError(item.l);
  const hasWErr = dimError(item.w);
  const hasHErr = dimError(item.h);
  const hasQErr = item.qty !== "" && parseInt(item.qty) <= 0;

  return (
    <div style={{ background: T.card, border: `1.5px solid ${(hasLErr || hasWErr || hasHErr || hasQErr) ? "#fca5a5" : T.border}`, borderRadius: T.radius, padding: 20, marginBottom: 14, boxShadow: T.shadow }}>
      ...
      {/* Pallet presets */}
      <div style={{ marginBottom: 12 }}>
        <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase", color: T.textDim, fontFamily: T.fontSans, marginBottom: 7 }}>
          Pallet Presets <span style={{ color: T.textFnt, fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>— prefills L × W, you add height & weight</span>
        </p>
        <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
          {PALLET_PRESETS.map(p => (
            <button key={p.label} onClick={() => { onChange("l", p.l); onChange("w", p.w); onChange("unit", p.unit); }}
              style={{ fontSize: 11, fontWeight: 600, color: T.navy, background: T.navyLt, border: `1.5px solid ${T.border}`, borderRadius: 8, padding: "5px 11px", cursor: "pointer", fontFamily: T.fontSans, transition: "background .15s" }}>
              📦 {p.label} ({p.l}×{p.w} cm)
            </button>
          ))}
        </div>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 76px", gap: 8 }}>
          <NumIn value={item.l} onChange={v => onChange("l", v)} placeholder="Length"  hasError={hasLErr} />
          <NumIn value={item.w} onChange={v => onChange("w", v)} placeholder="Width"   hasError={hasWErr} />
          <NumIn value={item.h} onChange={v => onChange("h", v)} placeholder="Height"  hasError={hasHErr} />
          <UnitSel value={item.unit} onChange={v => onChange("unit", v)} />
        </div>
        {(hasLErr || hasWErr || hasHErr) && (
          <p style={{ fontSize: 11, color: T.red, fontFamily: T.fontSans, marginTop: 5 }}>⚠ Dimensions must be greater than 0.</p>
        )}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        <div>
          <Lbl>Quantity (pcs)</Lbl>
          <NumIn value={item.qty} onChange={v => onChange("qty", v)} placeholder="1" hasError={hasQErr} />
          {hasQErr && <p style={{ fontSize: 11, color: T.red, fontFamily: T.fontSans, marginTop: 5 }}>⚠ Quantity must be at least 1.</p>}
        </div>
        <div>
          <Lbl>Gross Weight / pc (kg)</Lbl>
          <NumIn value={item.wt} onChange={v => onChange("wt", v)} placeholder="0.00" />
        </div>
      </div>

      {/* Item totals */}
      <div style={{ display: "flex", gap: 0, background: T.navyLt, borderRadius: 10, padding: "10px 14px", border: `1px solid ${T.border}` }}>
        {[
          { l: "CBM / pc",  v: fmtC(calc.cbmEach) },
          { l: "Total CBM", v: fmtC(calc.cbmTotal) },
          { l: "Total Wt",  v: `${fmtKg(calc.wtTotal)} kg` },
        ].map((x, i) => (
          <div key={x.l} style={{ display: "flex", flex: 1 }}>
            {i > 0 && <div style={{ width: 1, background: T.border, margin: "0 16px" }} />}
            <div>
...
/* ─── Divisor Selector ─────────────────────────────────────────────── */
function DivisorSelector({ divisor, onChange }) {
  return (
    <div style={{ background: T.card, border: `1.5px solid ${T.border}`, borderRadius: T.radius, padding: "16px 20px", marginBottom: 16, boxShadow: T.shadow }}>
      <p style={{ fontSize: 11, fontWeight: 700, color: T.textDim, letterSpacing: "0.09em", textTransform: "uppercase", fontFamily: T.fontSans, marginBottom: 12 }}>
        Air Freight Volumetric Divisor
      </p>
      <div style={{ display: "flex", gap: 10 }}>
        {DIVISORS.map(d => {
          const active = divisor === d.value;
          return (
            <button key={d.value} onClick={() => onChange(d.value)}
              style={{ flex: 1, textAlign: "left", background: active ? T.navyLt : T.cardAlt, border: `2px solid ${active ? T.navy : T.border}`, borderRadius: 12, padding: "12px 14px", cursor: "pointer", transition: "all .18s" }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: active ? T.navy : T.textMid, fontFamily: T.fontSans, marginBottom: 3 }}>÷{d.value}</p>
              <p style={{ fontSize: 11, color: active ? T.navyMd : T.textDim, fontFamily: T.fontSans, marginBottom: 2 }}>{d.sub}</p>
              <p style={{ fontSize: 10, color: T.textFnt, fontFamily: T.fontSans }}>{d.who}</p>
            </button>
          );
        })}
      </div>
      <div style={{ display: "flex", gap: 7, background: T.blueLt, border: `1px solid #bfdbfe`, borderRadius: 9, padding: "9px 12px", marginTop: 12 }}>
        <Info style={{ width: 13, height: 13, color: T.blue, flexShrink: 0, marginTop: 1 }} />
        <p style={{ fontSize: 11.5, color: "#1e40af", fontFamily: T.fontSans, lineHeight: 1.65 }}>
...
function CBMCalculator() {
  ...
  const calcs = useMemo(() => {
    const m = {};
    items.forEach(i => { m[i.id] = calcItem(i); });
    return m;
  }, [items]);
  ...
  const totals = useMemo(() => {
    ...
    const kgPerCBM = 1000000 / divisor;                // 166.67 or 200
    const volAir   = totalCBM * kgPerCBM;              // volumetric kg
    return {
      totalCBM, totalKg, totalQty,
      volAir,
      volSea:      totalCBM,
      chargeAir:   Math.max(totalKg, volAir),
      chargeSea:   Math.max(totalKg / 1000, totalCBM),
      kgPerCBM,
    };
  }, [items, calcs, divisor]);
  ...
  const handleCopy = () => {
    const kgPerCBM = (1000000 / divisor).toFixed(2);
    const txt = [
      "CBM Calculation — ONS Logistics", "",
      ...items.map((item, i) => {
        const c = calcs[item.id];
        return [
          `Package ${i + 1}${item.name ? ` — ${item.name}` : ""}`,
          `  ${item.l || 0}×${item.w || 0}×${item.h || 0} ${item.unit}  ×${item.qty || 0} pcs  ${item.wt || 0} kg/pc`,
          `  CBM/pc: ${fmtC(c.cbmEach)}  Total CBM: ${fmtC(c.cbmTotal)}  Total Wt: ${fmtKg(c.wtTotal)} kg`,
        ].join("\n");
      }),
      "", "─────────────────────────────",
      ...
      `Density        : ${(totals.totalKg / (totals.totalCBM || 1)).toFixed(1)} kg/m³`,
      "",
      `Divisor used   : ÷${divisor} (1 CBM = ${kgPerCBM} kg)`,
      `Vol. Wt (Air)  : ${fmtKg(totals.volAir)} kg`,
  ...
  const hasAnyError = items.some(item =>
    dimError(item.l) || dimError(item.w) || dimError(item.h) || (item.qty !== "" && parseInt(item.qty) <= 0)
  );
  ...
  return (
    <div id="calculator" style={{ display: "grid", gridTemplateColumns: "1fr 400px", gap: 24, maxWidth: 1180, margin: "0 auto", padding: "0 24px", alignItems: "start" }} className="cbm-grid">
      ...
      <div>
        {/* Divisor selector */}
        <DivisorSelector divisor={divisor} onChange={setDivisor} />

        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <h3 style={{ fontSize: 16, fontWeight: 700, color: T.navyDk, fontFamily: T.fontSans }}>Package Details</h3>
            <p style={{ fontSize: 12, color: T.textDim, marginTop: 2, fontFamily: T.fontSans }}>
              {items.length} package type{items.length !== 1 ? "s" : ""} · {totals.totalQty} total pieces
            </p>
          </div>
        ...
        {/* Validation banner */}
        {hasAnyError && (
          <div style={{ display: "flex", gap: 9, background: T.redLt, border: `1.5px solid #fca5a5`, borderRadius: 12, padding: "11px 15px", marginBottom: 14 }}>
            <AlertCircle style={{ width: 15, height: 15, color: T.red, flexShrink: 0, marginTop: 1 }} />
            <p style={{ fontSize: 13, color: "#991b1b", fontFamily: T.fontSans, lineHeight: 1.5 }}>
              Some packages have invalid values. Fix them to get accurate results — dimensions and quantity must be greater than 0.
            </p>
          </div>
        )}
        ...
        {items.length > 1 && (
          <div style={{ background: T.navy, borderRadius: T.radius, padding: "18px 22px", display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 16, boxShadow: T.shadowMd }}>
            {[
              { l: "Total Pieces", v: `${totals.totalQty} pcs` },
              { l: "Total CBM",    v: `${fmtC3(totals.totalCBM)} CBM` },
              { l: "Total Gross Wt", v: `${fmtKg(totals.totalKg)} kg` },
              { l: "Density",     v: totals.totalCBM > 0 ? `${(totals.totalKg / totals.totalCBM).toFixed(1)} kg/m³` : "—" },
            ].map(x => (
      ...
      <div style={{ position: "sticky", top: 88 }} className="cbm-sticky">
        ...
        <div style={{ background: T.card, borderRadius: T.radius, boxShadow: T.shadowMd, overflow: "hidden", marginBottom: 16, border: `1.5px solid ${T.border}` }}>
          ...
          <div style={{ padding: 22 }}>
            ...
            {[
              {
                icon: <Plane style={{ width: 14, height: 14, color: T.navy }} />,
                label: "Air Freight",
                sub: `÷${divisor} · 1 CBM = ${fmtDec(totals.kgPerCBM, 2)} kg`,
                actual: totals.totalKg,
                vol: totals.volAir,
                charge: totals.chargeAir,
                unit: "kg",
              },
              {
                icon: <Ship style={{ width: 14, height: 14, color: T.navy }} />,
                label: "Sea LCL",
                sub: "1 RT = 1 CBM or 1,000 kg",
                actual: totals.totalKg / 1000,
                vol: totals.volSea,
                charge: totals.chargeSea,
                unit: "RT",
              },
            ].map(m => (
              <div key={m.label} style={{ background: T.cardAlt, border: `1.5px solid ${T.border}`, borderRadius: 12, padding: "14px 16px", marginBottom: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
                  {m.icon}
                  <span style={{ fontSize: 12, fontWeight: 700, color: T.navy, fontFamily: T.fontSans }}>
                    {m.label} <span style={{ color: T.textDim, fontWeight: 500 }}>· {m.sub}</span>
                  </span>
                </div>
        ...
        {/* Density analysis */}
        <DensityCard totalKg={totals.totalKg} totalCBM={totals.totalCBM} divisor={divisor} />
        ...
        <div style={{ background: T.card, border: `1.5px solid ${T.border}`, borderRadius: T.radius, padding: 22, boxShadow: T.shadow, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, background: T.amberLt, border: `1px solid #fde68a`, borderRadius: 10, padding: "10px 13px", marginTop: 4 }}>
            ...
            <p style={{ fontSize: 11, color: "#92400e", lineHeight: 1.65, fontFamily: T.fontSans }}>
              <strong>Theoretical capacity only.</strong> Actual loading depends on cargo dimensions, stackability, palletisation, and carrier stowage rules. Verify with your freight forwarder.
            </p>
          </div>
        </div>
...
function Hero() {
  const stats = [
    ...
    { icon: <Calculator style={{ width: 15, height: 15 }} />, label: "IATA Standard",     sub: "÷6000 or ÷5000 divisor" },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48, alignItems: "center" }} className="hero-grid">
          <div>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
              ...
              <a href="/request-quote" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "#fff", borderRadius: 11, padding: "13px 26px", fontSize: 14, fontWeight: 700, textDecoration: "none", fontFamily: T.fontSans }}>
                Get a Freight Quote
              </a>
            </div>
          </div>
          ...
          <div className="hero-visual" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: 28, backdropFilter: "blur(8px)" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {[
                ...
                { label: "Vol. Wt (Air)",  value: "2,083",   sub: "÷6000 · chargeable", color: "#86efac" },
                { label: "Density",        value: "259.6",   sub: "kg/m³ · Standard", color: "#c4b5fd"  },
              ].map(x => (
...
function HowItWorks() {
  const steps = [
    { n: "01", icon: <Package style={{ width: 22, height: 22 }} />, title: "Enter Dimensions", body: "Type in your cargo length, width, and height. Switch between cm, mm, metres, inches, or feet — the calculator converts everything automatically. Use pallet presets for standard sizes." },
    { n: "02", icon: <Layers style={{ width: 22, height: 22 }} />,  title: "Add All Package Types", body: "Click 'Add Package' for each different SKU or box size. Mix units and quantities freely. Each row shows CBM per piece and totals instantly. Invalid dimensions show inline errors." },
    { n: "03", icon: <BarChart3 style={{ width: 22, height: 22 }} />, title: "Analyse Results", body: "Get total CBM, chargeable weight for air (÷6000 or ÷5000) and sea, cargo density classification, and container fit analysis with utilisation bars for 20ft GP, 40ft GP, and 40ft HC." },
  ];
  return (
...
function WhyMatters() {
  const features = [
    { icon: <TrendingUp style={{ width: 18, height: 18 }} />, color: T.blue,   title: "Avoid Freight Overcharges",      body: "Carriers bill whichever is higher — actual or volumetric weight. Knowing your density upfront lets you re-pack efficiently and negotiate rates." },
    ...
    { icon: <BadgeCheck style={{ width: 18, height: 18 }} />, color: T.navyMd, title: "IATA-Standard Calculations",    body: "Air volumetric weight supports both divisors: ÷6000 (standard air cargo, 166.67 kg/CBM) and ÷5000 (courier/express, 200 kg/CBM)." },
  ];
  return (
...
const FAQ_DATA = [
  ...
  { q: "How is volumetric weight calculated for air freight?",        a: "Air volumetric weight uses a divisor applied to the volume in cm³. The IATA standard for general air cargo is ÷6000 cm³/kg, meaning 1 CBM (1,000,000 cm³) = 166.67 kg. Express couriers (DHL, FedEx, UPS) use ÷5000, making 1 CBM = 200 kg. Airlines and couriers charge whichever is higher — the actual gross weight or the volumetric weight." },
  { q: "What is the difference between ÷6000 and ÷5000 divisors?",  a: "÷6000 is the IATA standard divisor for general air freight — 1 CBM equals 166.67 kg volumetrically. ÷5000 is used by express couriers such as DHL, FedEx, and UPS — 1 CBM equals 200 kg. Using the wrong divisor understates or overstates your chargeable weight, so always confirm which divisor your carrier applies before booking." },
  ...
  { q: "What is the difference between FCL and LCL?",               a: "FCL (Full Container Load) means you book an entire container regardless of fill level. LCL (Less than Container Load) means your cargo is consolidated with other shippers and you pay only for the CBM and weight you use, priced per Revenue Ton. FCL is generally more cost-efficient above 12–15 CBM; LCL suits smaller shipments." },
];

function FAQSection() {
  const [open, setOpen] = useState(0);

  /* Inject JSON-LD FAQ schema for SEO rich snippets */
  useEffect(() => {
    const script = document.createElement("script");
    script.type  = "application/ld+json";
    script.id    = "cbm-faq-schema";
    ...
    return () => { const el = document.getElementById("cbm-faq-schema"); if (el) el.remove(); };
  }, []);
    ...
    <div style={{ background: "#fff", padding: "72px 24px" }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {FAQ_DATA.map((item, i) => (
            <div key={i}
              style={{ borderRadius: 13, border: `1.5px solid ${open === i ? T.navy : T.border}`, background: open === i ? T.navyLt : T.card, overflow: "hidden", transition: "border-color .2s,background .2s" }}
            ...
            >
              <button onClick={() => setOpen(open === i ? null : i)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "17px 20px", background: "none", border: "none", cursor: "pointer", textAlign: "left", gap: 12 }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: open === i ? T.navy : T.text, fontFamily: T.fontSans, lineHeight: 1.5 }} itemProp="name">{item.q}</span>
              ...
              >
                <div className="acc-inner">
                  <div style={{ borderTop: `1px solid ${T.border}`, padding: "0 20px 17px" }}>
                    <p style={{ fontSize: 13.5, color: T.textMid, lineHeight: 1.85, fontFamily: T.fontSans, marginTop: 13 }} itemProp="text">{item.a}</p>
                  </div>
                </div>
              </div>
            </div>
