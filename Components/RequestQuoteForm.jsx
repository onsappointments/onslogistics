...
const RequestQuoteForm = forwardRef(
  ({ adminMode = false, setParentLoading }, ref) => {
    ...
    const validateForm = (showStatus) => {
      ...
      for (const field of required) {
        if (!form[field] || form[field].toString().trim() === "") {
          showStatus?.("warning", "Missing Field", `Please fill ${field}`);
          return false;
        }
      }
    ...
    const handleSubmit = async (showStatus) => {
      ...
      try {
        const endpoint = adminMode ? "/api/quotes/create" : "/api/quotes/init";
        ...
        // ===== CLIENT FLOW =====
        showStatus?.("success", "OTP Sent", "OTP sent to your email");
      ...
      >
              ...
              {form.fromLocationType === "ICD" && (
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={() =>
                      setTimeout(() => setFromIcdSuggestions([]), 200)
                    }
              ...
              {form.toLocationType === "ICD" && (
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={() =>
                      setTimeout(() => setToIcdSuggestions([]), 200)
                    }
                    placeholder="Type ICD..."
                    className="input-box"
                  />
        ...
        <section>
          <h2 className="text-2xl font-semibold mb-4">
            {adminMode ? "Client Details" : "Personal Details"}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* First Name */}
            <div>
              ...
              <div className="flex gap-2 items-center">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (val.length === 15) {
                      setGstinStatus(GSTIN_RE.test(val) ? "valid" : "invalid");
                    } else {
                      setGstinStatus(""); // still typing — show nothing
                    }
                  }}
                ...
                />
                {gstinStatus === "valid" && (
                  <span className="text-green-600 text-sm shrink-0">Valid</span>
                )}
                {gstinStatus === "invalid" && (
                  <span className="text-red-500 text-sm shrink-0">Invalid</span>
                )}
              </div>
            </div>
