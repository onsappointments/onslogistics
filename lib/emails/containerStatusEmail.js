...
const ETA_MESSAGES = {
  "Empty Picked Up": (d) => `The empty container pickup is scheduled for <strong>${d}</strong>.`,
  "Gate In": (d) => `Gate-in at the port is expected by <strong>${d}</strong>.`,
  "Loaded on Vessel": (d) => `Loading onto the vessel is expected by <strong>${d}</strong>.`,
  "Vessel Departed": (d) => `The vessel is expected to depart by <strong>${d}</strong>.`,
  "Arrived at Transshipment Port": (d) => `Arrival at the transshipment port is expected by <strong>${d}</strong>.`,
  "Vessel Arrived": (d) => `The vessel is expected to arrive at the port by <strong>${d}</strong>.`,
  "Discharged": (d) => `Discharge from the vessel is expected by <strong>${d}</strong>.`,
  "Gate Out": (d) => `Gate-out from the port is expected by <strong>${d}</strong>.`,
  "Delivered": (d) => `Delivery is expected by <strong>${d}</strong>.`,
};

const ACTUAL_MESSAGES = {
  "Empty Picked Up": (d) => `The empty container was picked up on <strong>${d}</strong>.`,
  "Gate In": (d) => `The container completed gate-in at the port on <strong>${d}</strong>.`,
  "Loaded on Vessel": (d) => `The container was loaded onto the vessel on <strong>${d}</strong>.`,
  "Vessel Departed": (d) => `The vessel departed on <strong>${d}</strong>.`,
  "Arrived at Transshipment Port": (d) => `The container arrived at the transshipment port on <strong>${d}</strong>.`,
  "Vessel Arrived": (d) => `The vessel arrived at the port on <strong>${d}</strong>.`,
  "Discharged": (d) => `The container was discharged from the vessel on <strong>${d}</strong>.`,
  "Gate Out": (d) => `The container completed gate-out from the port on <strong>${d}</strong>.`,
  "Delivered": (d) => `The shipment was delivered on <strong>${d}</strong>.`,
};
...
export function buildStatusEmailHtml({
  ...
  const etaRow = eta
    ? detailRow("Estimated Date", fmt(eta), "#b45309")       // was "Estimated Arrival"
    : "";
    
  const departureRow = actualDeparture
    ? detailRow("Confirmed Date", fmt(actualDeparture), "#047857", !remarks)  // was "Actual Date"
    : "";
