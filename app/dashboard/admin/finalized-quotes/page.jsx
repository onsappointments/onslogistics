...
const pipeline = [
  ...
  // Search filter — runs after both lookups so all fields are available
  ...(search
    ? [
        {
          $match: {
            $or: [
              ...
              {
                "clientQuote.firstName": {
                  $regex: escapeRegex(search),
                  $options: "i",
                },
              },
              {
                "clientQuote.lastName": {
                  $regex: escapeRegex(search),
                  $options: "i",
                },
              },
              {
                "job.beNumber": { $regex: escapeRegex(search), $options: "i" },
              },
              {
                "job.mblNumber": { $regex: escapeRegex(search), $options: "i" },
              },
              {
                "job.hblNumber": { $regex: escapeRegex(search), $options: "i" },
              },
              {
                "job.awbNumber": { $regex: escapeRegex(search), $options: "i" },
              },
            ],
          },
        },
  ...
  // Re-group job back into array so the active_jobs size check still works
  {
    $group: {
      _id: "$_id",
      root: { $first: "$$ROOT" },
  ...
  },
  {
    $replaceRoot: {
      newRoot: {
        $mergeObjects: ["$root", { job: "$job" }],
      },
    },
  },

  show === "active_jobs"
    ? { $match: { job: { $elemMatch: { $ne: null } } } }
    : { $match: { $or: [{ job: { $size: 0 } }, { job: [null] }] } },

  { $sort: { createdAt: -1 } },
];

  const quotes = await TechnicalQuote.aggregate(pipeline);
        ...
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Review quotes that are sent to client / approved — and optionally
              those converted into jobs.
            </p>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2">
              ...
              {search && (
                <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200 text-sm font-semibold">
                  🔍 &quot;{search}&quot;
                </span>
              )}
            </div>
        ...
        <div className="px-8 py-6 border-b border-white/40 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Click <span className="font-semibold">View</span> to open full
              breakdown.
            </p>
          </div>
        ...
        {count === 0 ? (
          <div className="p-14 text-center">
            ...
            <p className="text-lg font-semibold text-gray-900">
              {search ? `No results for "${search}"` : "No finalized quotes"}
            </p>
            <p className="text-sm text-gray-600 mt-1">
              {search
                ? "Try a different search term or clear the search."
                : "Try switching tabs/filters."}
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            ...
            <div className="px-8 py-4 border-t border-gray-100 text-xs text-gray-500">
              💡 Tip: Use tabs to switch between Approved / Sent / Converted
              jobs.
            </div>
          </div>
        )}
