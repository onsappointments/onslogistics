...
export default function CompletedJobsPage() {
  ...
  const fetchJobs = async (type = shipmentType) => {
    ...
    setErr("");
    try {
      const url = `/api/admin/jobs?status=completed${type ? `&shipmentType=${type}` : ""}`;
      ...
      console.error(e);
      setErr("Failed to fetch completed jobs");
      setJobs([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-7xl mx-auto p-10 space-y-8">
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-blue-100 p-8">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
          <div>
            <h1 className="text-4xl font-bold text-gray-900">Completed Jobs</h1>
            <p className="text-gray-600 mt-2">
              All jobs Completed. Click any row to view full
              details.
            </p>
      ...
      <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <FilterButton
              label="All"
              completed={shipmentType === ""}
              onClick={() => setShipmentType("")}
            />
            <FilterButton
              label="Import"
              completed={shipmentType === "import"}
              onClick={() => setShipmentType("import")}
            />
            <FilterButton
              label="Export"
              completed={shipmentType === "export"}
              onClick={() => setShipmentType("export")}
            />
            <FilterButton
              label="Courier"
              completed={shipmentType === "courier"}
...
function FilterButton({ label, completed, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
